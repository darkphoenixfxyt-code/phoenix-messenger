...
function appendBubble(message, convo, scroll = true) {
  ...
  const attachmentHtml = renderAttachment(message.attachment, mine);
  const textHtml = message.deleted ? 'Message deleted' : escapeHtml(message.text || '');
  const bodyHtml = `${textHtml ? `<div>${textHtml}</div>` : ''}${attachmentHtml}`;
...
async function sendAttachment(file) {
  if (!activeId || !currentUser || !file) return;
  ...
  try {
    const path = `conversations/${activeId}/${currentUser.uid}/${Date.now()}-${safeFileName(file.name)}`;
    ...
    alert(`Upload failed: ${error.code || error.message || 'check Firebase Storage rules'}`);
  } finally {
