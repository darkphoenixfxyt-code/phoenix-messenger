...
function typingDocId(convoId = activeId) {
...
function renderMessageTools(message, mine) {
  ...
  return `
    <div class="msg-tools">
      ...
      ${mine ? `<button class="msg-tool danger" onclick="deleteMessage('${escapeHtml(message.id)}')">Delete</button>` : ''}
...
async function editMessage(messageId) {
  ...
  if (!text) return;
  await updateDoc(doc(db, 'conversations', activeId, 'messages', messageId), {
    text,
...
async function toggleReaction(messageId, emoji) {
  ...
  await updateDoc(doc(db, 'conversations', activeId, 'messages', messageId), { reactions });
}
