...
async function startDmWithUser(other, firstMessage = '') {
  ...
  const blockedA = await getDoc(doc(db, 'blocks', `block_${currentUser.uid}_${otherUid}`));
  const blockedB = await getDoc(doc(db, 'blocks', `block_${otherUid}_${currentUser.uid}`));
  if (blockedA.exists() || blockedB.exists()) {
    showToast('This DM is blocked.', 'error');
    return;
  }
