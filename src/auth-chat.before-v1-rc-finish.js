...
async function saveProfile() {
  ...
  currentProfile = { ...currentProfile, username: desired, usernameLower: lower, displayName, bio, avatarURL, bannerURL, pronouns, location, website, status, customStatus, theme, notificationSound: sound };
  username = desired;
  localStorage.setItem('phoenix_username', desired);
...
function hasServerPermission(permission, serverId = activeServerId) {
  ...
  const permissions = getActiveServer()?.rolePermissions?.[role] || ROLE_DEFAULTS[role]?.permissions || [];
...
function listenToMessages(convoId) {
  ...
  activeConvoUnsub = onSnapshot(q, snapshot => {
    ...
    showToast(friendlyError(error, 'Could not load messages.'), 'error');
  });
}
...
function startChannelSync(serverId) {
  ...
  channelsUnsub = onSnapshot(channelQuery, snapshot => {
    ...
    showToast(friendlyError(error, 'Could not load server channels.'), 'error');
  });
  ...
  serverMembersUnsub = onSnapshot(collection(db, 'servers', serverId, 'members'), snapshot => {
    ...
    Promise.all(serverMembers.map(async member => {
      const profileSnap = await getDoc(doc(db, 'users', member.uid)).catch(() => null);
      return profileSnap?.exists() ? { ...member, profile: profileSnap.data() } : member;
    })).then(enriched => {
    ...
    console.error('Server member sync failed:', error);
    showToast(friendlyError(error, 'Could not load server members.'), 'error');
  });
}
...
function syncVoiceChannelMembers(serverId, voiceChannels) {
  ...
  voiceChannels.forEach(channel => {
    ...
    voiceChannelMemberUnsubs[channel.id] = onSnapshot(
      ...
      error => console.error('Voice member sync failed:', error)
    );
  });
}
...
function renderMessageTools(message, mine) {
  ...
  return `
    <div class="msg-tools">
      ...
      <button class="msg-tool" onclick="togglePinMessage('${escapeHtml(message.id)}')">${message.pinned ? 'Unpin' : 'Pin'}</button>
  ...
  if (String(attachment.type || '').startsWith('image/')) {
    return `
      <a class="attachment-image-link" href="${url}" target="_blank" rel="noopener">
        <img class="attachment-image" src="${url}" alt="${name}" loading="lazy" decoding="async">
      </a>
...
async function getLiveKitToken(roomName, metadata = '') {
  ...
  try {
    ...
    response = await fetch('/.netlify/functions/livekit-token', {
      method: 'POST',
      headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(data.error || (response.status === 404
      ? 'LiveKit token function was not found. Redeploy Netlify after adding netlify/functions/livekit-token.js.'
...
function wireLiveKitRoom(room) {
  room
    ...
    .on(RoomEvent.Disconnected, () => setLiveKitStatus('Disconnected'))
...
async function legacyJoinVoiceChannel(serverId, channelId, channelName = 'Voice channel') {
  ...
  try {
    ...
    alert(`Could not join voice: ${error.message || error.name || 'microphone permission denied'}`);
  ...
  voiceMembersUnsub = onSnapshot(collection(db, 'servers', serverId, 'voiceChannels', channelId, 'members'), snapshot => {
    ...
    console.error('Active voice member sync failed:', error);
  ...
  try {
    ...
    card.innerHTML = `
      ...
      <div class="profile-card-body">
        ...
        <div class="profile-card-status">${escapeHtml(profile.customStatus || profile.status || 'offline')}</div>
...
function sendBugReport() {
  ...
  const subject = `Phoenix Messenger beta bug report (${APP_VERSION})`;
  const body = [
    'What happened?',
    what,
    '',
...
async function runGlobalSearch() {
  ...
  convos.forEach(convo => {
    (convo.messages || []).filter(message => String(message.text || '').toLowerCase().includes(term)).slice(0, 10).forEach(message => {
      rows.push({ type: 'Message', title: message.text, meta: `${message.senderName || 'User'} in ${convo.name}`, action: `openConvo('${escapeHtml(convo.id)}');closeGlobalSearch()` });
    });
  });
  (activeChannel?.messages || []).filter(message => String(message.text || '').toLowerCase().includes(term)).slice(0, 10).forEach(message => {
    rows.push({ type: 'Message', title: message.text, meta: `${message.senderName || 'User'} in #${activeChannel.name}`, action: 'closeGlobalSearch()' });
  });
  ...
  channels.filter(channel => String(channel.name || '').toLowerCase().includes(term)).forEach(channel => rows.push({ type: 'Channel', title: `#${channel.name}`, meta: getActiveServer()?.name || 'Server', action: `openChannel('${escapeHtml(channel.id)}');closeGlobalSearch()` }));
...
function setupRoleDragAndDrop() {
  ...
  hierarchy.addEventListener('drop', event => {
    ...
    if (!target || !dragged || target === dragged || draggedRole === 'owner') return;
...
function canKickServerMember(member) {
  ...
  const myRole = getMyServerMember()?.role || 'member';
  const roleRank = { owner: 4, admin: 3, mod: 2, member: 1 };
  if (!['owner', 'admin'].includes(myRole)) return false;
  return (roleRank[myRole] || 0) > (roleRank[member.role || 'member'] || 1);
}
    ...
    ${members.map(member => {
      ...
      return `
        <div class="member-card" onclick="openRichProfile('${escapeHtml(member.uid)}')">
          ...
          ${canKickServerMember(member) ? `<button class="member-kick" onclick="event.stopPropagation();kickServerMember('${escapeHtml(member.uid)}')">Kick</button>` : ''}
        </div>`;
    }).join('')}`;
