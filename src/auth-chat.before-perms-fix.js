...
async function findUserByUsername(value) {
  ...
  if (!snap.empty) {
    ...
    return { id: docSnap.id, uid: docSnap.data().uid || docSnap.id, ...docSnap.data() };
  }
  ...
  results.innerHTML = `
    <div class="user-card">
      ...
      <div class="user-card-actions">
        <button class="btn-mini primary" onclick="startDmFromSearch('${escapeHtml(uid)}')">Message</button>
        <button class="btn-mini" onclick="sendFriendRequest('${escapeHtml(uid)}')" ${self || alreadyFriend ? 'disabled' : ''}>Add</button>
