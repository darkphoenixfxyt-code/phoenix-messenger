...
const DESKTOP_AUTH_TIMEOUT_MS = 2 * 60 * 1000;
const DESKTOP_AUTH_POLL_MS = 1500;
...
let founderModeEnabled = localStorage.getItem('phoenix_founder_mode') === 'true';
let auroraModeEnabled = localStorage.getItem('phoenix_aurora') === 'true';
let replyDraft = null;
...
function getDesktopAuthQuery() {
  const params = new URLSearchParams(window.location.search);
  const sessionId = params.get('session') || '';
  return {
    enabled: params.get('desktopAuth') === '1' && Boolean(sessionId),
    sessionId,
    params
  };
}

function clearDesktopAuthQuery() {
  const url = new URL(window.location.href);
  url.searchParams.delete('desktopAuth');
  url.searchParams.delete('desktopExternalAuth');
  url.searchParams.delete('session');
  window.history.replaceState({}, '', url);
}

function createDesktopAuthSessionId() {
  if (window.crypto?.randomUUID) return window.crypto.randomUUID();
  return `desktop-auth-${Date.now()}-${Math.random().toString(36).slice(2, 12)}`;
}

function desktopAuthDoc(sessionId) {
  return doc(db, 'desktopAuthSessions', sessionId);
}

function desktopAuthRedirectKey(sessionId) {
  return `phoenix_desktop_auth_${sessionId}`;
}

async function writeDesktopAuthState(sessionId, payload) {
  await setDoc(desktopAuthDoc(sessionId), payload, { merge: true });
}

async function publishDesktopAuthSuccess(sessionId, result) {
  const credential = GoogleAuthProvider.credentialFromResult(result);
  if (!credential?.idToken || !credential?.accessToken) {
    const error = new Error('Google login finished, but no desktop credential was returned.');
    error.code = 'auth/missing-desktop-credential';
    throw error;
  }
  await writeDesktopAuthState(sessionId, {
    status: 'completed',
    provider: 'google',
  ...
  sessionStorage.removeItem(desktopAuthRedirectKey(sessionId));
  setAuthError('Desktop login approved. Return to the Phoenix Messenger app.');
  showToast('Desktop login approved. Return to the app.', 'success');
  clearDesktopAuthQuery();
}

async function publishDesktopAuthFailure(sessionId, error, status = 'error') {
  await writeDesktopAuthState(sessionId, {
    status,
    provider: 'google',
    message: friendlyError(error, 'Google login failed.'),
...
async function bootstrapDesktopBrowserAuth() {
  const { enabled, sessionId } = getDesktopAuthQuery();
  if (!enabled || isDesktopApp()) return false;

  showLanding(false);
  showAuth(true);
  lockMessenger(true);

  const redirectKey = desktopAuthRedirectKey(sessionId);

  try {
    const redirectResult = await getRedirectResult(auth);
    if (redirectResult) {
      await publishDesktopAuthSuccess(sessionId, redirectResult);
      return true;
    }

    if (sessionStorage.getItem(redirectKey) === 'started') {
      const cancelled = new Error('Google login was cancelled before it finished.');
      cancelled.code = 'auth/cancelled-popup-request';
      await publishDesktopAuthFailure(sessionId, cancelled, 'cancelled');
      setAuthError(friendlyError(cancelled, 'Google login cancelled.'));
      return true;
    }

    sessionStorage.setItem(redirectKey, 'started');
    setAuthError('Redirecting to Google sign-in for desktop login...');
    await signInWithRedirect(auth, googleProvider);
    return true;
  } catch (error) {
    await publishDesktopAuthFailure(sessionId, error, /cancel|closed/i.test(String(error?.code || error?.message || '')) ? 'cancelled' : 'error');
    setAuthError(friendlyError(error, 'Google login failed.'));
    return true;
  }
}

async function waitForDesktopAuthCompletion(sessionId) {
  const startedAt = Date.now();
  const ref = desktopAuthDoc(sessionId);

  while (Date.now() - startedAt < DESKTOP_AUTH_TIMEOUT_MS) {
    await delay(DESKTOP_AUTH_POLL_MS);
    const snapshot = await getDoc(ref);
    if (!snapshot.exists()) continue;
    const data = snapshot.data() || {};

    if (data.status === 'completed' && data.idToken && data.accessToken) {
      const credential = GoogleAuthProvider.credential(data.idToken, data.accessToken);
      await signInWithCredential(auth, credential);
      await deleteDoc(ref).catch(() => {});
      showToast('Desktop login complete.', 'success');
      return;
    }

    if (data.status === 'cancelled') {
      await deleteDoc(ref).catch(() => {});
      const cancelled = new Error(data.message || 'Google login was cancelled.');
      cancelled.code = 'auth/cancelled-popup-request';
      throw cancelled;
    }
    ...
    if (data.status === 'error') {
      await deleteDoc(ref).catch(() => {});
      const failed = new Error(data.message || 'Desktop login failed.');
      failed.code = data.errorCode || 'auth/desktop-auth-failed';
      throw failed;
    }
  }

  await deleteDoc(ref).catch(() => {});
  const timeout = new Error('Desktop login timed out. Try again.');
  timeout.code = 'auth/timeout';
  throw timeout;
}

async function handleDesktopGoogleLogin() {
  const sessionId = createDesktopAuthSessionId();
  const authUrl = new URL(window.location.origin + window.location.pathname);
  authUrl.searchParams.set('desktopExternalAuth', '1');
  authUrl.searchParams.set('desktopAuth', '1');
  authUrl.searchParams.set('session', sessionId);

  await setDoc(desktopAuthDoc(sessionId), {
    status: 'pending',
    provider: 'google',
    createdAt: serverTimestamp(),
    createdAtMs: Date.now(),
    expiresAtMs: Date.now() + DESKTOP_AUTH_SESSION_TTL_MS,
    userAgent: String(navigator.userAgent || '').slice(0, 300)
  });

  setAuthError('Continue Google sign-in in your browser, then return here.');
  showToast('Opening Google sign-in in your browser...', 'info');
  window.location.assign(authUrl.toString());
  await waitForDesktopAuthCompletion(sessionId);
}
...
function requestMacDownload() {
  const link = document.createElement('a');
  link.href = '/downloads/Phoenix%20Messenger_1.4.0-beta_aarch64.dmg';
  link.download = 'Phoenix Messenger_1.4.0-beta_aarch64.dmg';
  document.body.appendChild(link);
  link.click();
  link.remove();
  showToast('macOS desktop preview download started.', 'success');
}
...
function isLocalFounder(uid = currentUser?.uid) {
  return founderModeEnabled && Boolean(currentUser?.uid) && uid === currentUser.uid;
}

function renderFounderMode() {
  $('topFounderBadge')?.classList.toggle('visible', founderModeEnabled);
  document.body.classList.toggle('founder-mode', founderModeEnabled);
}

function applyAuroraMode() {
  document.body.classList.toggle('aurora-mode', auroraModeEnabled);
}

function openEasterEgg(type = 'phoenix') {
  const title = $('easterEggTitle');
  const content = $('easterEggContent');
  if (!title || !content) return;
  const panels = {
    phoenix: {
      title: 'Phoenix has risen.',
      html: '<div class="easter-phoenix"><span>◆</span></div><h2 class="easter-heading">Phoenix has risen.</h2><p class="easter-copy easter-quote">“From the ashes, we build again.”</p>'
    },
    founder: {
      title: 'Founder Mode unlocked',
      html: '<div class="easter-phoenix"><span>F</span></div><h2 class="easter-heading">Founder Mode unlocked.</h2><p class="easter-copy">A local Founder badge now appears beside your Phoenix username on this device.</p>'
    },
    ai: {
      title: 'Phoenix AI',
      html: '<div class="easter-phoenix"><span>AI</span></div><h2 class="easter-heading">Phoenix AI is not online yet.</h2><p class="easter-copy">Soon you’ll be able to ask Phoenix to summarize chats, help moderate servers, and generate replies.</p>'
    },
    launcher: {
      title: 'Phoenix Launcher linked.',
      html: '<div class="easter-phoenix"><span>◆</span></div><h2 class="easter-heading">Phoenix Launcher linked.</h2><div class="launcher-checklist"><div class="launcher-check">Offline AI</div><div class="launcher-check">Messenger</div><div class="launcher-check">Voice</div><div class="launcher-check">Servers</div><div class="launcher-check">Phoenix Hub</div></div>'
    },
    credits: {
      title: 'Developer credits',
      html: '<div class="easter-phoenix"><span>◆</span></div><h2 class="easter-heading">Phoenix Messenger</h2><div class="launcher-checklist"><div class="launcher-check">Built by Prince</div><div class="launcher-check">Powered by Firebase, Cloudinary, and LiveKit</div><div class="launcher-check">Phoenix AI coming soon</div></div>'
    }
  };
  const panel = panels[type] || panels.phoenix;
  title.textContent = panel.title;
  content.innerHTML = panel.html;
  $('easterEggOverlay')?.classList.add('open');
}

function closeEasterEgg() {
  $('easterEggOverlay')?.classList.remove('open');
}

function closeEasterEggOutside(event) {
  if (event.target === $('easterEggOverlay')) closeEasterEgg();
}

function unlockFounderMode() {
  founderModeEnabled = true;
  localStorage.setItem('phoenix_founder_mode', 'true');
  renderFounderMode();
  openEasterEgg('founder');
}

function toggleAuroraMode() {
  auroraModeEnabled = !auroraModeEnabled;
  localStorage.setItem('phoenix_aurora', String(auroraModeEnabled));
  applyAuroraMode();
  showToast(auroraModeEnabled ? 'Aurora Mode awakened.' : 'Aurora Mode resting.', 'success');
}

function handleSecretCommand(value) {
  const command = String(value || '').trim().toLowerCase();
  if (command === 'phoenix') openEasterEgg('phoenix');
  else if (command === '440255') unlockFounderMode();
  else if (command === '/ai') openEasterEgg('ai');
  else if (command === '/launcher') openEasterEgg('launcher');
  else return false;
  return true;
}
...
function setupEasterEggTriggers() {
  ...
  const logo = $('phoenixLauncherMark');
  const activateLogo = () => {
    logoClicks += 1;
    clearTimeout(logoResetTimer);
    logoResetTimer = setTimeout(() => { logoClicks = 0; }, 5000);
  ...
  const activateVersion = () => {
    ...
    versionResetTimer = setTimeout(() => { versionClicks = 0; }, 5000);
    if (versionClicks < 7) return;
    versionClicks = 0;
    openEasterEgg('credits');
  };
  document.querySelectorAll('[data-version-easter]').forEach(label => {
    label.addEventListener('click', activateVersion);
...
function friendlyError(error, fallback = 'Something went wrong.') {
  ...
  if (code === 'auth/popup-blocked') return 'The login popup was blocked. Allow popups or try again.';
  if (['auth/popup-closed-by-user', 'auth/cancelled-popup-request'].includes(code)) return 'Google login was cancelled before it finished.';
  if (code === 'auth/network-request-failed') return 'Network error. Check your connection and try again.';
  if (code === 'auth/timeout') return 'Google login took too long. Try again.';
...
function openEmojiPicker() {
  ...
  if ($('uploadServerEmojiBtn')) $('uploadServerEmojiBtn').style.display = activeMode === 'server' && ['owner', 'admin'].includes(getMyServerMember()?.role) ? '' : 'none';
  loadServerEmojis();
...
async function loadServerEmojis() {
  ...
  const snapshot = await getDocs(collection(db, 'servers', activeServerId, 'emojis')).catch(() => null);
  serverEmojis = snapshot?.docs.map(item => ({ id: item.id, ...item.data() })) || [];
  const grid = $('emojiPickerGrid');
  if (!grid || !$('emojiPickerOverlay')?.classList.contains('open')) return;
  serverEmojis.forEach(emoji => {
    grid.insertAdjacentHTML('beforeend', `<button class="emoji-choice custom-emoji-choice" title=":${escapeHtml(emoji.name)}:" onclick="insertEmoji(':${escapeHtml(emoji.name)}:')"><img src="${escapeHtml(emoji.url)}" alt=":${escapeHtml(emoji.name)}:"></button>`);
  });
}

function chooseServerEmoji() {
  if (activeMode !== 'server' || !['owner', 'admin'].includes(getMyServerMember()?.role)) {
    showToast('Only server owners and admins can upload emojis.', 'error');
    return;
  }
  $('serverEmojiUploadInput')?.click();
}

async function uploadServerEmoji(file) {
  if (!file || !activeServerId) return;
  if (file.size > 1024 * 1024) {
    showToast('Server emojis must be smaller than 1 MB.', 'error');
    return;
  }
  const name = String(prompt('Emoji name (letters, numbers, underscores)') || '').trim().toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 24);
  if (!name) return;
  try {
    showToast('Uploading server emoji...', 'info');
    const url = await uploadImage(file);
    await setDoc(doc(db, 'servers', activeServerId, 'emojis', name), {
      name, url, createdBy: currentUser.uid, createdAt: serverTimestamp()
    });
    await loadServerEmojis();
    showToast(`:${name}: added.`, 'success');
  } catch (error) {
    showToast(friendlyError(error, 'Could not upload server emoji.'), 'error');
  } finally {
    if ($('serverEmojiUploadInput')) $('serverEmojiUploadInput').value = '';
  }
}
...
function renderMessageText(text) {
  ...
  serverEmojis.forEach(emoji => {
    ...
    html = html.split(token).join(`<img class="custom-emoji-inline" src="${escapeHtml(emoji.url)}" alt="${escapeHtml(token)}" title="${escapeHtml(token)}">`);
  });
  return html;
}
...
async function handleGoogleLogin() {
  if (desktopAuthInFlight) return;
  setAuthError('');
  setButtonLoading('googleLoginBtn', true, 'Connecting...');
  try {
    desktopAuthInFlight = true;
    if (isDesktopApp()) {
      await handleDesktopGoogleLogin();
    } else {
      await signInWithPopup(auth, googleProvider);
    }
  } catch (error) {
    if (!isDesktopApp() && error?.code === 'auth/popup-blocked') {
...
async function openCommunityDiscovery() {
  $('communityDiscoveryOverlay')?.classList.add('open');
  const list = $('communityDiscoveryList');
  if (list) list.innerHTML = '<div class="friend-empty">Finding communities...</div>';
  const snapshot = await getDocs(query(collection(db, 'servers'), where('public', '==', true), limit(50))).catch(error => {
    showToast(friendlyError(error, 'Could not load public servers.'), 'error');
    return null;
  });
  publicServers = snapshot?.docs.map(item => ({ id: item.id, ...item.data() })) || [];
  renderCommunityDiscovery();
}

function closeCommunityDiscovery() {
  $('communityDiscoveryOverlay')?.classList.remove('open');
}

function closeCommunityDiscoveryOutside(event) {
  if (event.target === $('communityDiscoveryOverlay')) closeCommunityDiscovery();
}

function renderCommunityDiscovery() {
  const list = $('communityDiscoveryList');
  if (!list) return;
  const term = String($('communitySearchInput')?.value || '').trim().toLowerCase();
  const visible = publicServers
    .filter(server => !term || `${server.name || ''} ${server.description || ''}`.toLowerCase().includes(term))
    .sort((a, b) => Number(Boolean(b.featured)) - Number(Boolean(a.featured)) || Number(b.memberCount || 0) - Number(a.memberCount || 0));
  list.innerHTML = visible.length ? visible.map(server => `
    <article class="discovery-card">
      <h3>${server.featured ? 'Featured · ' : ''}${escapeHtml(server.name || 'Community')}</h3>
      <p class="settings-note">${escapeHtml(server.description || 'A Phoenix community.')}</p>
      <div class="discovery-actions"><span class="btn-mini">${Number(server.memberCount || 0)} members</span><button class="btn-mini primary" onclick="joinPublicServer('${escapeHtml(server.id)}')">${(server.memberIds || []).includes(currentUser?.uid) ? 'Open' : 'Join'}</button></div>
    </article>`).join('') : '<div class="friend-empty">No public servers match this search.</div>';
}

async function joinPublicServer(serverId) {
  const server = publicServers.find(item => item.id === serverId) || servers.find(item => item.id === serverId);
  if (!server || !currentUser) return;
  if (!(server.memberIds || []).includes(currentUser.uid)) {
    const banSnap = await getDoc(doc(db, 'servers', serverId, 'bans', currentUser.uid));
    if (banSnap.exists()) {
  ...
  if (!servers.some(item => item.id === serverId)) servers.unshift(server);
  closeCommunityDiscovery();
  openServer(serverId);
}

function getActiveMessages() {
  return (activeMode === 'server' ? activeChannel : convos.find(item => item.id === activeId))?.messages || [];
}

function summarizeMessages(messages = getActiveMessages()) {
  const usable = messages.filter(message => !message.deleted && message.text).slice(-40);
  if (!usable.length) return 'There are no recent text messages to summarize.';
  const senders = [...new Set(usable.map(message => message.senderName || 'User'))];
  const topics = [...new Set(usable.flatMap(message => String(message.text).toLowerCase().match(/[a-z]{5,}/g) || []))]
    .filter(word => !['about', 'there', 'their', 'would', 'could', 'should', 'message', 'phoenix'].includes(word))
    .slice(0, 6);
  return `${usable.length} recent messages from ${senders.slice(0, 5).join(', ')}.${topics.length ? ` Main topics: ${topics.join(', ')}.` : ''} Latest: “${usable.at(-1).text.slice(0, 180)}”`;
}

function buildPhoenixAiResponse(promptText) {
  const prompt = String(promptText || '').toLowerCase();
  const messages = getActiveMessages();
  if (prompt.includes('summarize server')) {
    const server = getActiveServer();
    if (!server) return 'Open a server first so I can summarize it.';
    return `${server.name} has ${Number(server.memberCount || serverMembers.length)} members, ${channels.filter(item => item.type === 'text').length} text channels, and ${channels.filter(item => item.type === 'voice').length} voice channels. ${summarizeMessages(messages)}`;
  }
  if (prompt.includes('summarize')) return summarizeMessages(messages);
  if (prompt.includes('suggest') || prompt.includes('reply')) {
    const latest = messages.filter(message => !message.deleted && message.text).at(-1);
    return latest ? `Suggested reply: “Thanks for the update. I’m looking into ${latest.text.slice(0, 80)} now.”` : 'Start a conversation and I can suggest a reply.';
  }
  if (prompt.includes('moderation') || prompt.includes('moderate')) {
    const flagged = messages.filter(message => /(spam|scam|threat|hate|harass)/i.test(message.text || '')).length;
    return flagged ? `I found ${flagged} recent message${flagged === 1 ? '' : 's'} that may need moderator review. No action was taken.` : 'No obvious moderation risks were found in the recent loaded messages. Human review is still recommended.';
  }
  return `Phoenix AI is running locally for this preview. I can summarize the current chat or server, suggest a reply, and provide a moderation review.`;
}

function openPhoenixAi(initialPrompt = '') {
  $('phoenixAiOverlay')?.classList.add('open');
  if (!$('phoenixAiThread')?.children.length) {
    $('phoenixAiThread').innerHTML = '<div class="ai-message">Phoenix AI online in local assistant mode. Ask me about the current conversation.</div>';
  }
  if (initialPrompt) runPhoenixAi(initialPrompt);
  setTimeout(() => $('phoenixAiInput')?.focus(), 50);
}

function closePhoenixAi() {
  $('phoenixAiOverlay')?.classList.remove('open');
}

function closePhoenixAiOutside(event) {
  if (event.target === $('phoenixAiOverlay')) closePhoenixAi();
}

function runPhoenixAi(promptOverride = '') {
  const input = $('phoenixAiInput');
  const promptText = String(promptOverride || input?.value || '').trim();
  if (!promptText) return;
  openPhoenixAi();
  const thread = $('phoenixAiThread');
  thread.insertAdjacentHTML('beforeend', `<div class="ai-message user">${escapeHtml(promptText)}</div><div class="ai-message ai-typing" id="phoenixAiTyping"><span></span><span></span><span></span></div>`);
  if (input) input.value = '';
  thread.scrollTop = thread.scrollHeight;
  setTimeout(() => {
    $('phoenixAiTyping')?.remove();
    thread.insertAdjacentHTML('beforeend', `<div class="ai-message">${escapeHtml(buildPhoenixAiResponse(promptText))}</div>`);
    thread.scrollTop = thread.scrollHeight;
  }, 650);
}
...
function openServer(serverId) {
  ...
  loadServerEmojis().then(renderActiveConvoMessages).catch(() => {});
...
function appendBubble(message, convo, scroll = true) {
  ...
  const textHtml = deleted ? '<span class="deleted-text">Message deleted</span>' : renderMessageText(message.text || '');
...
async function sendMessage() {
  ...
  if (/^@phoenix\b/i.test(text)) {
    box.innerText = '';
    stopTyping();
    openPhoenixAi(text.replace(/^@phoenix\b[:,]?\s*/i, '') || 'summarize conversation');
    return;
  }
...
function toggleScreenShareFullscreen() {
  ...
  if (!viewer?.classList.contains('visible')) {
    showToast('Start or watch a screen share first.', 'info');
    return;
  }
  ...
  try {
    ...
    card.innerHTML = `
      ...
      <div class="profile-card-body">
        ...
        <div class="profile-badges"><span class="profile-badge">Phoenix</span>${isLocalFounder(uid) ? '<span class="profile-badge founder">Founder</span>' : ''}${(profile.badges || []).map(badge => `<span class="profile-badge">${escapeHtml(badge)}</span>`).join('')}</div>
...
function openServerSettings() {
  ...
  if ($('serverSettingsPublic')) $('serverSettingsPublic').checked = Boolean(server.public);
...
async function saveServerSettings(trigger = null) {
  ...
  try {
    ...
    await updateDoc(doc(db, 'servers', activeServerId), {
      ...
      public: Boolean($('serverSettingsPublic')?.checked),
      ...
      return `
        <div class="member-card" onclick="openRichProfile('${escapeHtml(member.uid)}')">
          ...
          <div class="member-card-main">
            ...
            <div class="activity-label">${escapeHtml(getMemberActivity(member))}</div>
          </div>
...
function getMemberActivity(member) {
  const voiceEntry = Object.entries(voiceChannelMembers).find(([, members]) => members.some(item => item.uid === member.uid));
  if (voiceEntry) return `In Voice Channel · ${channels.find(item => item.id === voiceEntry[0])?.name || 'Voice'}`;
  if (member.uid === currentUser?.uid && screenShareActive) return 'Watching Screen Share';
  if (getVisibleStatus(member.profile, member.uid) === 'idle') return 'Idle';
  return member.profile?.activity || 'Playing Phoenix';
}

function openServerDashboard() {
  const server = getActiveServer();
  const grid = $('serverDashboardGrid');
  if (!server || !grid) return;
  const textChannels = channels.filter(item => item.type === 'text');
  const voiceUsers = Object.values(voiceChannelMembers).reduce((count, members) => count + members.length, 0);
  const messageEstimate = textChannels.reduce((count, channel) => count + Number(channel.messageCount || 0), 0);
  const created = server.createdAt?.toDate ? server.createdAt.toDate() : null;
  const ageDays = created ? Math.max(1, Math.ceil((Date.now() - created.getTime()) / 86400000)) : 1;
  grid.innerHTML = `
    <div class="dashboard-stat"><strong>${Number(server.memberCount || serverMembers.length)}</strong><span>Members</span></div>
    <div class="dashboard-stat"><strong>${messageEstimate}</strong><span>Messages tracked</span></div>
    <div class="dashboard-stat"><strong>${voiceUsers}</strong><span>Active voice users</span></div>
    <div class="dashboard-stat"><strong>${textChannels.length}</strong><span>Text channels</span></div>
    <div class="dashboard-stat"><strong>${Math.round(Number(server.memberCount || 1) / ageDays * 10) / 10}</strong><span>Members per day</span></div>
    <div class="dashboard-stat"><strong>${server.public ? 'Public' : 'Private'}</strong><span>Discovery status</span></div>`;
  $('serverDashboardOverlay')?.classList.add('open');
}

function closeServerDashboard() {
  $('serverDashboardOverlay')?.classList.remove('open');
}

function closeServerDashboardOutside(event) {
  if (event.target === $('serverDashboardOverlay')) closeServerDashboard();
}
...
await bootstrapDesktopBrowserAuth();
...
updateDownloadCounter();
if (Math.random() < 0.05) {
  setTimeout(() => showToast('Rare transmission received: Phoenix is watching the network.', 'info'), 900);
}
...
$('phoenixAiInput')?.addEventListener('keydown', e => {
  if (e.key === 'Enter') runPhoenixAi();
});
...
$('serverEmojiUploadInput')?.addEventListener('change', event => {
  uploadServerEmoji(event.target.files?.[0]);
});
...
Object.assign(window, {
  ...
  closePhoenixAi,
  closePhoenixAiOutside,
  closeCommunityDiscovery,
  closeCommunityDiscoveryOutside,
  closeServerDashboard,
  closeServerDashboardOutside,
  closeEasterEgg,
  closeEasterEggOutside,
  closeNotificationsPanel,
