...
async function saveProfile() {
  ...
  currentProfile = { ...currentProfile, username: desired, usernameLower: lower, displayName, bio, avatarURL, bannerURL, pronouns, location, website, status, theme, notificationSound: sound };
  username = desired;
  localStorage.setItem('phoenix_username', desired);
...
async function getLiveKitToken(roomName, metadata = '') {
  ...
  try {
    ...
    throw new Error(`Voice token request failed: ${error.message || 'network error'}`);
  }
